...
class Administration(commands.Cog):
    ...
    async def mute(
        ...
        if duration is None:
            ...
            if (r := re.search(r"(\d+)\s*d", duration)) is not None:
                total += timedelta(days=int(r.group(1)))
            if (r := re.search(r"(\d+)\s*h", duration)) is not None:
                total += timedelta(hours=int(r.group(1)))
            if (r := re.search(r"(\d+)\s*m", duration)) is not None:
                total += timedelta(minutes=int(r.group(1)))
            ...
            try:
                ...
                return await ctx.send(f"**Error:** **{member}** is already muted.")
    ...
    async def unmute(self, ctx: commands.Context, member: discord.Member):
        ...
        try:
            ...
            return await ctx.send(f"**{member}** is not currently muted.")
    ...
    async def on_member_join(self, member: discord.Member):
        if self.db.fetch_member(member).muted:
            try:
                role = next(filter(lambda x: x.name == "Muted", member.guild.roles))
                await member.add_roles(role)
            except StopIteration:
